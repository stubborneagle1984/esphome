...
void setup() {
  ...
  //   build_path: build/esphome-con-antenna
  //   area: ''
  //   platformio_options: {}
  //   includes: []
  //   libraries: []
  ...
  App.pre_setup("esphome-con-antenna", "esphome-con-antenna", "", "", __DATE__ ", " __TIME__, false);
  // text_sensor:
  // fan:
  // logger:
  //   level: DEBUG
  ...
  wifi_wificomponent->set_use_address("10.4.1.184");
  {
  ...
  wifi_wifiap.set_ssid("Esphome-con-antenna");
  wifi_wifiap.set_password("75ObNtLwvREr");
  ...
  ota_otacomponent->set_auth_password("beba92a4fb540398e82ac44832a421b5");
  ...
  api_apiserver->set_noise_psk({3, 124, 184, 66, 157, 229, 176, 202, 9, 149, 57, 128, 104, 163, 45, 70, 91, 72, 45, 178, 176, 167, 36, 204, 44, 22, 178, 72, 211, 219, 228, 57});
  // sensor:
  // i2c:
  //   sda: 21
  //   scl: 22
  ...
  sensor_lambdafilter = new sensor::LambdaFilter([=](float x) -> optional<float> {
      #line 85 "/config/esphome-sperimentazione-con-antenna.yaml"
      return min(max(2 * (x + 100.0), 0.0), 100.0);
  });
  ...
  roll->set_template([=]() -> optional<float> {
      #line 122 "/config/esphome-sperimentazione-con-antenna.yaml"
  ...
  pitch->set_template([=]() -> optional<float> {
      #line 130 "/config/esphome-sperimentazione-con-antenna.yaml"
